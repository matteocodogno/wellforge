...
const PORT_FILE = process.env.FORGE_VC_PORT_FILE || null;
...
function preferredPort() {
  if (process.env.FORGE_VC_PORT) return Number(process.env.FORGE_VC_PORT);
  if (PORT_FILE) {
    try {
...
const HOST = process.env.FORGE_VC_HOST || '127.0.0.1';
const URL_HOST = process.env.FORGE_VC_URL_HOST || (HOST === '127.0.0.1' ? 'localhost' : HOST);
const SESSION_DIR = process.env.FORGE_VC_DIR || '/tmp/forge-vc';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const FORGE_VERSION = readForgeVersion();
// Design-system theme applied to the frame (mantine | mui | shadcn | wireframe | default).
// The designer picks it from the project's real component library so mockups look like
// the app being built, not a neutral wireframe. Resolved to themes/<name>.css.
const THEME = process.env.FORGE_VC_THEME || 'default';
let ownerPid = process.env.FORGE_VC_OWNER_PID ? Number(process.env.FORGE_VC_OWNER_PID) : null;
...
// Persisted alongside the port (FORGE_VC_TOKEN_FILE) so a restart keeps the
// same key and an already-open tab's cookie still validates.
const TOKEN_FILE = process.env.FORGE_VC_TOKEN_FILE || null;
...
function initialToken() {
  if (process.env.FORGE_VC_TOKEN) {
    return { value: process.env.FORGE_VC_TOKEN, source: 'env' };
  }
  if (TOKEN_FILE) {
    try {
...
let COOKIE_NAME = 'forge-vc-key-' + PORT; // refined to the actual bound port in onListen
...
<html>
<head><meta charset="utf-8"><title>WellForge Visual Companion</title>
<style>
...
<body><!-- BRANDING --><h1>WellForge Visual Companion</h1>
<p>Waiting for the designer to push a screen...</p></body></html>`);
}
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening WellForge Visual Companion</title></head>
<body>
<script>
try { sessionStorage.setItem('forge-vc-session-key', ${jsonKey}); } catch (e) {}
location.replace('/');
</script>
</body>
</html>`;
...
// Resolve the requested design-system theme to CSS injected into the frame's <head>.
// Themes only override the CSS custom properties the frame already consumes, so a
// missing/unknown theme silently falls back to the built-in default palette.
function readTheme(name) {
  if (!name || name === 'default') return '';
  const file = path.join(__dirname, 'themes', path.basename(name) + '.css');
  try { return fs.readFileSync(file, 'utf-8'); } catch (e) { return ''; }
}
const themeCss = readTheme(THEME);
const themeInjection = themeCss ? '<style>\n' + themeCss + '\n</style>' : '';

// Bake the theme into the frame template once (theme is constant for the process).
const frameTemplate = fs
  .readFileSync(path.join(__dirname, 'frame-template.html'), 'utf-8')
  .split('<!-- THEME -->').join(themeInjection);
...
function readForgeVersion() {
  ...
  try {
    ...
    // Plugin manifest not resolvable from here — fall through to 'unknown'.
  }
  return 'unknown';
}
...
// Text-only brand — no external image, no telemetry (internal WellForge tooling).
function brandMarkup() {
  const label = THEME && THEME !== 'default'
    ? 'WellForge Visual Companion · ' + escapeHtmlText(THEME)
    : 'WellForge Visual Companion';
  return '<div class="brand"><span class="brand-copy">' + label +
    ' <span style="opacity:.6">v' + escapeHtmlText(FORGE_VERSION) + '</span></span></div>';
}
...
// browser is already connected. Override the launcher with FORGE_VC_OPEN_CMD.
let browserOpened = false;
function maybeOpenBrowser() {
  if (browserOpened) return;
  browserOpened = true;
  if (!process.env.FORGE_VC_OPEN) return; // opt-in: only after the user approves the companion
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') return;
  if (clients.size > 0) return; // the user already opened it
  const url = companionUrl(); // must carry the key or the gate 403s it
  const cp = require('child_process');
  // Operator-provided launcher: run as given (this env var is trusted operator input).
  if (process.env.FORGE_VC_OPEN_CMD) {
    try { cp.exec(process.env.FORGE_VC_OPEN_CMD + ' ' + JSON.stringify(url), () => {}); } catch (e) { /* best effort */ }
    return;
  }
...
// override with FORGE_VC_IDLE_TIMEOUT_MS (start-server.sh: --idle-timeout-minutes).
const IDLE_TIMEOUT_MS = (() => {
  const ms = Number(process.env.FORGE_VC_IDLE_TIMEOUT_MS);
...
const LIFECYCLE_CHECK_MS = (() => {
  const ms = Number(process.env.FORGE_VC_LIFECYCLE_CHECK_MS);
...
function startServer() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource === 'env') {
        console.error('Server failed to bind: preferred port is in use and FORGE_VC_TOKEN is set; refusing fallback with explicit token');
        process.exit(1);
      }
      triedFallback = true;
      PORT = randomPort();
