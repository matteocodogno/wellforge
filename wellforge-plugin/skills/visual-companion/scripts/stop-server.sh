#!/usr/bin/env bash
# WellForge Visual Companion — stop the server and clean up.
# Usage: stop-server.sh <session_dir>
#
# Kills the server process. Only deletes session directory if it's
# under /tmp (ephemeral). Persistent directories (.forge/design/) are
...
command_has_server_id() {
  local pid="$1"
  local expected="$2"
  local expected_arg="--forge-vc-server-id=$expected"
  if [[ -r "/proc/$pid/cmdline" ]]; then
    local arg
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if ! is_forge_vc_server "$pid"; then
