...
| `.github/workflows/quality.yml` | when `ci == github`: **calls** the reusable gates `{{ gates_repo }}/.github/workflows/*.yml@{{ gates_ref }}` (never inlines gate logic). **Three gates are called unconditionally, outside the `rigor` branch**: `linear-history.yml`, `commit-lint.yml` and `security-floor.yml`. None of the three can be repaired after the fact — history rewrites and credential rotation are not fixes — so they do not belong to a tier. This row used to say only `linear-history.yml` was tier-independent, and the templates matched it: `security-floor.yml` was called in the `spike` branch and nowhere else, `commit-lint.yml` only in the `mvp`/`production` branch, so each was the other's blind spot and an `mvp` project had no secret scan at all. Tier-conditional is only the *work*: `production`/`mvp` call `quality-<stack>.yml` (passing `rigor` so `mvp` coverage is advisory); `spike` gets a build-sanity job instead, and that job is **advisory** (`continue-on-error: true` + a step summary), matching the `rigor-tiers` skill, which puts build in the advisory column at `spike`. When the preset has backing services (`hono-react` with `db == postgres`), the backend job also passes `compose-file` + `env-file` so integration tests get a real database; `pretest-command` ships commented out (a fresh scaffold has no migrations) |
